    ...
    <div class="mb-4 rounded-xl bg-gray-50 p-4 ring-1 ring-gray-950/5 dark:bg-white/5 dark:ring-white/10">
        ...
        <div x-show="exportOpen" x-cloak class="mt-3 space-y-3">
            <p class="text-xs text-gray-500 dark:text-gray-400">
                Pick which languages to include, then download a zip with one text file per language ({{ '{lang}.txt' }}) containing just that language's category name.
            </p>
            ...
            <div class="flex justify-end">
                <x-filament::button
                    ...
                    @click="$wire.downloadCategoryExport({{ Illuminate\Support\Js::from($categoryId) }}, Object.keys(exportSelected).filter(code => exportSelected[code]))"
                >
                    Download zip
                </x-filament::button>
            </div>
                ...
                <div class="mt-3 border-t border-gray-100 pt-3 dark:border-white/5">
                    <div class="flex flex-wrap items-center justify-between gap-2">
                        <div class="flex items-center gap-2">
                            <span class="text-xs font-medium text-gray-400 dark:text-gray-500">Name</span>

                            {{-- Warning/success shades colored via inline styles reading Filament's own
                                 --warning-*/--success-* CSS custom properties, not
                                 bg-warning-50/text-success-700-style utility classes - see
                                 service-translation-details.blade.php for the same reasoning. --}}
                            @if ($language['isDefault'])
                            ...
                            @elseif ($language['pending'])
                                ...
                                <x-filament::button
                                    ...
                                    wire:click="cancelTranslation({{ Illuminate\Support\Js::from($categoryId) }}, {{ Illuminate\Support\Js::from($language['code']) }})"
                            ...
                            @if ($language['retranslated'] ?? false)
                                <span class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium" style="background-color: rgba(var(--primary-500), .1); color: rgb(var(--primary-700))" title="The source name changed, so this was automatically re-translated">
                        ...
                        @if (! $language['isDefault'] && ! $language['pending'])
                            <x-filament::button
                                ...
                                wire:click="translateCategory({{ Illuminate\Support\Js::from($categoryId) }}, {{ Illuminate\Support\Js::from($language['code']) }})"
