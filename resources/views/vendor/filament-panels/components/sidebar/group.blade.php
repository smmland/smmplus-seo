...
@php
    ...
    // Not a Filament feature - this app's own addition (PanelNotificationService), summing
    // whatever badge each item in the group already carries (getNavigationBadge()) rather than
    // querying notifications again here, so this stays generic to any future per-item badge, not
    // just notification counts.
    $groupBadgeTotal = collect($items)
        ->sum(fn ($item) => is_numeric($item->getBadge()) ? (int) $item->getBadge() : 0);
@endphp
