<div x-data="{ langSelected: {{ Illuminate\Support\Js::from($languages->mapWithKeys(fn ($l) => [$l['code'] => true])->all()) }} }">
    <p class="mb-4 text-sm text-gray-500 dark:text-gray-400">
        {{ $selectedCount }} categor{{ $selectedCount === 1 ? 'y' : 'ies' }} selected. The zip groups files into one folder per category id (e.g. <code class="rounded bg-gray-100 px-1 dark:bg-white/10">{{ $fileExample }}</code>), each containing just that language's {{ $fieldLabel }} text.
    </p>
